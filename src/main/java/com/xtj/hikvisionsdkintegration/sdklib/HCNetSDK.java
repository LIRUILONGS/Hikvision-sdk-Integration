package com.xtj.hikvisionsdkintegration.sdklib;
...
public interface HCNetSDK extends Library {
    ...
    public static final int PICNAME_PREFIX = 32;      /*图片名自定义前缀长度*/
    /******************************************************************/
...
//时间点
	...
	仅支持当前分辨率的抓图*/
        ...
        public short wPicQuality = 0;            /* 图片质量系数 0-最好 1-较好 2-一般 */
    }
...
14-白炽灯 (IncandescentLamp)，15-暖光灯(Warm Light Lamp)，16-自然光(Natural Light) */
    ...
    public static class NET_DVR_SCHED_CAPTURECFG extends Structure { // 通道录像参数配置(9000扩展)
        ...
        public byte[] byRes1 = new byte[3];
        public NET_DVR_CAPTURE_DAY[] struCaptureDay = new NET_DVR_CAPTURE_DAY[MAX_DAYS];  //全天抓图计划，数组0~6分别对应星期一~星期日

        public tempStructure[] tempStructure = new tempStructure[MAX_DAYS];
        public NET_DVR_CAPTURE_DAY struCaptureHoliday = new NET_DVR_CAPTURE_DAY(); //假日抓图计划

        public NET_DVR_CAPTURE_SCHED[] struHolidaySched = new NET_DVR_CAPTURE_SCHED[MAX_TIMESEGMENT_V30]; //时间段抓图布防计划，数组0~6分别对应星期一~星期日

        public int dwRecorderDuration;
        public int dwDelayTime; // 抓图延时时间 0- 无延时， 1-3秒， 2-4秒， 3-5秒， 4-10秒， 5-30秒， 6-60秒， 7- 120秒，8-300秒

        public byte[] byRes = new byte[36];

        @Override
        public String toString() {
            return "NET_DVR_SCHED_CAPTURECFG{" +
                    "dwSize=" + dwSize +
                    ", byEnable=" + byEnable +
                    ", byRes1=" + Arrays.toString(byRes1) +
                    ", struCaptureDay=" + Arrays.toString(struCaptureDay) +
                    ", tempStructure=" + Arrays.toString(tempStructure) +
                    ", struCaptureHoliday=" + struCaptureHoliday +
                    ", struHolidaySched=" + Arrays.toString(struHolidaySched) +
                    ", dwRecorderDuration=" + dwRecorderDuration +
                    ", dwDelayTime=" + dwDelayTime +
    ...
    public static class NET_DVR_TIMING_CAPTURE extends Structure {
        public NET_DVR_JPEGPARA      struJpegPara = new NET_DVR_JPEGPARA();  // 定时抓图图片质量
        public int                  dwPicInterval = 3 ;  //定时抓图时间间隔
        public byte[]               byRes = new byte[3];
    ...
    public static class  NET_DVR_JPEG_CAPTURE_CFG extends Structure {
        ...
        public  NET_DVR_TIMING_CAPTURE  struTimingCapture = new NET_DVR_TIMING_CAPTURE(); //定时抓图参数配置

        public  NET_DVR_EVENT_CAPTURE   struEventCapture = new NET_DVR_EVENT_CAPTURE(); //事件抓图参数配置

        public byte[]                   byStreamType= new byte[64]; //抓图码流类型：0- 主码流，1- 子码流

        public byte[]                     byRes3= new byte[19];

        @Override
        public String toString() {
            return "NET_DVR_JPEG_CAPTURE_CFG{" +
                    "dwSize=" + dwSize +
                    ", struTimingCapture=" + struTimingCapture +
                    ", struEventCapture=" + struEventCapture +
                    ", byStreamType=" + Arrays.toString(byStreamType) +
                    ", byRes3=" + Arrays.toString(byRes3) +
                    '}';
        }
    }

    public static final int MAX_PIC_EVENT_NUM = 32; //事件抓图的事件类型个数
    public static final int MAX_ALARMIN_CAPTURE = 16; //
    public static class NET_DVR_EVENT_CAPTURE  extends Structure {
        public NET_DVR_JPEGPARA      struJpegPara;  // 事件抓图图片质量

        public int                  dwPicInterval;  //事件抓图时间间隔
        public byte[]               byRes = new byte[59];
        public byte[]               byCapTimes = new byte[59]; //抓图张数

        public NET_DVR_REL_CAPTURE_CHAN[]       struRelCaptureChan = new  NET_DVR_REL_CAPTURE_CHAN[MAX_PIC_EVENT_NUM];
        public NET_DVR_REL_CAPTURE_CHAN[]       struAlarmInCapture = new  NET_DVR_REL_CAPTURE_CHAN[MAX_ALARMIN_CAPTURE];


    }

    public  static  class  NET_DVR_REL_CAPTURE_CHAN extends Structure  {
        public byte[]        byChan = new byte[16];
        public byte[]        byRes = new byte[20];
